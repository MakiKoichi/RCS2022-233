...
Kd = 256;                                % データ長
K  = Kp + Kd;                            % 合計のシンボルフレーム
Phi = 1/N;                               % レイリーフェージングの分散
Es = 1;                                  % 平均送信エネルギー密度
cx = sqrt(Es/2);                         % QPSK信号の実部・虚部の絶対値
R = 25;                                  % Zadoff-Chu系列の根
l = -1;                                  % Zadoff-Chu系列生成に関する整数
Xp = sqrt(Es)*MYzadoffChumat(R,M,Kp,l);  % パイロット行列
rho = 0.7;                               % 隣接する受信アンテナ素子間の相関係数
SNRdB = 23;                              % 信号対雑音比 SNR [dB]
N0    = 10^(-SNRdB/10);                  % 加法性白色ガウス雑音の分散
T = 32;                                  % BiGaBP処理における最大繰り返し数

%% 学習設定

a = 4;                       % シグモイド関数のゲイン
MINI_BATCH = 3000;           % ミニバッチ数
BATCH_SIZE = 100;            % バッチサイズ
EPOCH = 1;                   % 学習エポック数
STEP_SIZE  = 5e-3;           % SGDのステップサイズ(学習率)
...
for minibatch = init_minibatch:MINI_BATCH
   ...
   %% パラメータの更新
    ...
    alpha = max(0,alpha); % ランプ関数の出力として更新
    beta  = max(0,beta ); % ランプ関数の出力として更新
    gamma = max(0,gamma); % ランプ関数の出力として更新
